...
<p align="center">
  <strong>VNX = Verifiable Network Exchange</strong>
</p>
...
The VNX (**Verifiable Network Exchange**) Paid Micro-Swarm is a deterministic, competition-grade system that dispatches tasks (originally trading signals, now generalized to **any verifiable claim**) to a swarm of specialized agent workers, selects the highest-confidence winner, pays them in HBAR on Hedera mainnet, and produces a **cryptographically verifiable proof receipt** with SHA-256 hashes and live mirror-node confirmation.

**VNX + BitLattice works extremely well across real-world verification domains** (energy data → carbon credit retirement, supply chain provenance, RWA attestations, model output verification, compliance, etc.). The competitive swarm of "BitLattice provers" + deterministic scoring + on-chain HCS anchoring (shared Vera Lattice topic) + receipts gives you trust-minimized, independently auditable outcomes for physical or digital claims. See the companion `hedera-vnx-wv-carbon-swarm` for a complete production example of the pattern applied to West Virginia energy production/usage leading to verified carbon retirements on the same HCS topic.
...
Full runbook: [docs/TESTNET_SWARM.md](docs/TESTNET_SWARM.md)

Peak observed (2026-06-18): **1,154 TPS** on testnet topic `0.0.9227346`.
